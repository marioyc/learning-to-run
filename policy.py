...
class Policy(object):
    ...
    def core(self, obs, prev_internal_state, prev_actions):
        ...
        cell_input = tf.nn.bias_add(
            tf.zeros([batch_size, self.cell_input_dim]), b)
                ...
                for i, (act_dim, act_type) in enumerate(self.env_spec.act_dims_and_types):
                    ...
                    if self.env_spec.is_discrete(act_type):
                        cell_input += tf.matmul(
                            tf.one_hot(prev_actions[i], act_dim), w)
    ...
    def get_kl(self, my_logits, other_logits):
        ...
        for i, (act_dim, act_type) in enumerate(self.env_spec.act_dims_and_types):
            ...
            if self.env_spec.is_discrete(act_type):
                ...
                my_kl = tf.reduce_sum(
                    my_probs * (my_log_probs - other_log_probs), -1)
            elif self.env_spec.is_box(act_type):
...
class MLPPolicy(Policy):
    ...
    def get_cell(self):
        ...
        def mlp(cell_input, prev_internal_state):
            w1 = tf.get_variable(
                'w1', [self.cell_input_dim, self.internal_dim])
    ...
    def multi_step(self, all_obs, initial_state, all_actions):
        ...
        for i, (act_dim, act_type) in enumerate(self.env_spec.act_dims_and_types):
            ...
            if self.env_spec.is_discrete(act_type):
                reshaped_act.append(tf.reshape(
                    act, [time_length * batch_size]))
        ...
        internal_states = tf.reshape(
            internal_states, [time_length, batch_size, -1])

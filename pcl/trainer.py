...
flags.DEFINE_integer('replay_batch_size', None,
...
flags.DEFINE_bool('use_online_batch', True,
...
flags.DEFINE_string('save_file_prefix', 'model', 'prefix for model files')
flags.DEFINE_string(
    'load_path', '', 'path of saved model to load (if none in save_dir)')
...
class Trainer(object):
    ...
    def run(self):
        ...
        init_fn(self.sess, saver)

        logging.info('hparams:\n%s', self.hparams_string())

        model_step = self.sess.run(self.model.global_step)
        ...
        for step in xrange(1 + self.num_steps):
            ...
            if step % self.validation_frequency == 0:
                saver.save(self.sess, os.path.join(FLAGS.save_dir, FLAGS.save_file_prefix),

...
class Reinforce(objective.Objective):
    ...
    def get(self, rewards, pads, values, final_values,
        ...
        rewards = not_pad * \
            tf.reshape(rewards, [seq_length, -1, self.num_samples])
        log_probs = not_pad * \
            tf.reshape(sum(log_probs), [seq_length, -1, self.num_samples])
        ...
        loss = -tf.stop_gradient(rewards_and_bonus -
                                 baseline) * total_log_probs
